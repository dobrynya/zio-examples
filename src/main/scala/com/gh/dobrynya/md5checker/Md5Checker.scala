...
object Md5Checker extends ZIOAppDefault :
  ...
  private[md5checker] def printInfo(d: FileDescription) =
    d match
      ...
      case FileDescription(url, _, _, Some(error)) =>
        s"File at $url won't be downloaded due to an error: $error!"
  
  private[md5checker] def calculateMd5(description: FileDescription): ZIO[HttpClient, Exception, FileDescription] =
  ...
  private[md5checker] def program(url: String) =
      readFileDescriptions(url).partition(_.valid) 
        .flatMap(_.mapZIOParUnordered(8)(calculateMd5).merge(_).foreach(file => Console.printLine(printInfo(file))))
  ...
  override def run: ZIO[ZIOAppArgs & Scope, Throwable, Unit] =
    ZIOAppArgs.getArgs.flatMap(args => program(args.headOption.getOrElse("file:urls.txt")))
      .provideSome[ZIOAppArgs & Scope](zioHttpClient)
